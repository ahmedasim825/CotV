...
class OllamaClient {
  ...
  Uri get _generateEndpoint => Uri.parse('$baseUrl/api/generate');
  ...
  /// Loads the model if it is not resident, and resets Ollama's unload
  /// timer to [keepAlive].
  ///
  /// Ollama drops a model from memory after five minutes idle, and
  /// reloading 2.2GB costs about 17s to first token against 2.4s warm —
  /// measured on this machine, not estimated. That difference is the whole
  /// argument for a local engine, so leaving it to chance would mean the
  /// default route is usually the slow one.
  ///
  /// Goes to the native endpoint because `keep_alive` is *not* honoured on
  /// the OpenAI-compatible one. That was measured too: the same field sent
  /// to /v1/chat/completions left the timer at five minutes. With no body
  /// beyond the model and the timer, this returns as soon as the model is
  /// resident rather than generating anything.
  ///
  /// Fire and forget by construction — nothing waits on it, and a failure
  /// only means the next turn pays a load it would have paid anyway.
    ...
    try {
      await _http
          .post(
            ...
            body: jsonEncode({'model': model, 'keep_alive': keepAlive}),
          )
          .timeout(warmTimeout);
    } on Object {
      // Ollama stopped between the probe and here, or the machine has no
      // memory for the model. Both surface properly on the turn itself.
    }
  }
...
///
/// Generous enough to cover a cold model load, which is the one thing here
/// that legitimately takes tens of seconds: 17s measured on this machine,
/// and a slower disk would take longer.
const Duration localStallTimeout = Duration(seconds: 90);

/// How long Ollama keeps the model resident after a turn.
///
/// Its own default is five minutes, which is shorter than the gap between
/// two things a person asks an assistant, so the default route would pay
/// the reload most times it ran.
const String keepAlive = '30m';

/// How long to wait on the warm-up call. It returns as soon as the model is
/// resident, so this only has to cover the load itself.
const Duration warmTimeout = Duration(seconds: 120);
