...
final localBrainStatusProvider = Provider<Future<LocalBrainStatus> Function()>(
  (ref) => () async {
    ...
    // Reaching a ready status means a turn is about to run locally, so the
    // model is about to be needed either way — this only decides whether it
    // is loaded before the request or during it, and resets the unload
    // timer so the *next* turn is warm too. Not awaited: the streaming
    // request is started immediately after and Ollama serialises the two
    // onto the same load, so waiting here would only add latency to the
    // first turn without saving any.
    if (status.isReady) unawaited(client.warm());
